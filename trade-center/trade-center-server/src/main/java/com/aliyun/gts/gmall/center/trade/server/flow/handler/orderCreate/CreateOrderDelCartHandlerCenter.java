...
@Component
public class CreateOrderDelCartHandlerCenter extends AdapterHandler<TOrderCreate> {
    ...
    public void handle(TOrderCreate inbound) {
        ...
        // 购物车下单
        List<ItemSkuId> items = order.getMainOrders().stream().flatMap(main -> main.getSubOrders().stream())
                .map(sub -> sub.getItemSku().getSellerIdItemSkuId())
        ...
        batch.setPayMode(order.getPayMode());
