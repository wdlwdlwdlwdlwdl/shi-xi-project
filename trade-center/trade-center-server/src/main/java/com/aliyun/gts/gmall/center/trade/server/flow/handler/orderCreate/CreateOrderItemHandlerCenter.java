...
@Component
public class CreateOrderItemHandlerCenter extends AdapterHandler<TOrderCreate> {
    ...
    public void handle(TOrderCreate inbound) {
        ...
        for (MainOrder mainOrder : order.getMainOrders()) {
            for (SubOrder subOrder : mainOrder.getSubOrders()) {
                ItemSkuId itemSkuId = new ItemSkuId(subOrder.getItemSku().getItemId(), subOrder.getItemSku().getSkuId(),
                        mainOrder.getSeller().getSellerId(),subOrder.getSkuQuoteId(),subOrder.getCityCode());
                idList.add(itemSkuId);
            }
        }
        ...
        for (MainOrder main : order.getMainOrders()) {
            for (SubOrder sub : main.getSubOrders()) {
                ItemSku item = itemMap.get(
                    new ItemSkuId(sub.getItemSku().getItemId(), sub.getItemSku().getSkuId(), main.getSeller().getSellerId())
                );
                //对代客下单逻辑进行处理
